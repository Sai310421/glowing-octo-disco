...
enum ENUM_ZR_ADAPTIVE_SCHEME
{
   ZR_SCHEME_MANUAL = 0,              // use InpZoneAnchorMode / InpZoneWidthFactor as-is
   ZR_SCHEME_FIXED_FIRST = 1,         // fixed width + first-entry anchor
   ZR_SCHEME_ROLLING_EXPANDING = 2,   // rolling anchor + expanding width
   ZR_SCHEME_AUTO_STATE = 3           // AMOS optimal: fixed in range, rolling in danger
};
...
input double                InpZoneWidthFactor           = 1.10;      // manual fallback / legacy setting
input ENUM_ZR_ANCHOR_MODE   InpZoneAnchorMode            = ZR_ANCHOR_ROLLING; // manual fallback / legacy setting

input group "=== AMOS ZR Adaptive A/B Optimizer ==="
input ENUM_ZR_ADAPTIVE_SCHEME InpZRAdaptiveScheme        = ZR_SCHEME_AUTO_STATE;
input double                InpZRStandardZoneFactor      = 1.00;      // B: Fixed + FirstAnchor
input ENUM_ZR_ANCHOR_MODE   InpZRStandardAnchorMode      = ZR_ANCHOR_FIRST;
input double                InpZRDefenseZoneFactor       = 1.10;      // A: Rolling + Expanding
input ENUM_ZR_ANCHOR_MODE   InpZRDefenseAnchorMode       = ZR_ANCHOR_ROLLING;
input double                InpZRDefenseDDTrigger        = 3.0;       // switch to defensive scheme when DD >= this
input double                InpZRDefenseSpreadRatio      = 0.70;      // switch when spread >= maxSpread * ratio
input int                   InpZRDefenseMinTurn          = 2;         // switch after this rescue turn count
input bool                  InpZRTrendUsesDefenseScheme  = true;
...
//====================================================================
// Adaptive A/B scheme selector
//====================================================================
int GetZRDangerScore()
{
   int score = 0;

   if(g_zr.state == STATE_CHAOS) score += 4;
   else if(g_zr.state == STATE_TREND && InpZRTrendUsesDefenseScheme) score += 1;

   double dd = GetEquityDDPercent();
   if(dd >= InpZRHardEquityDDPercent * 0.80) score += 4;
   else if(dd >= InpZRDefenseDDTrigger)      score += 2;

   double sp = GetCurrentSpreadPips();
   if(InpZRMaxSpreadPips > 0.0)
   {
      if(sp >= InpZRMaxSpreadPips) score += 4;
      else if(sp >= InpZRMaxSpreadPips * InpZRDefenseSpreadRatio) score += 2;
   }

   if(g_zr.recovery_started && g_zr.turn_count >= InpZRDefenseMinTurn) score += 1;
   if(g_zr.policy == ZR_POLICY_EXIT_ONLY) score += 2;

   return MathMin(score, 10);
}

bool ZRUseDefenseScheme()
{
   if(InpZRAdaptiveScheme == ZR_SCHEME_ROLLING_EXPANDING) return true;
   if(InpZRAdaptiveScheme == ZR_SCHEME_FIXED_FIRST)       return false;
   if(InpZRAdaptiveScheme == ZR_SCHEME_MANUAL)
      return (InpZoneAnchorMode == ZR_ANCHOR_ROLLING && InpZoneWidthFactor > 1.0);

   // AUTO_STATE: normal rescue starts with B, danger rescue switches to A.
   if(g_zr.state == STATE_CHAOS) return true;
   if(g_zr.state == STATE_TREND && InpZRTrendUsesDefenseScheme && GetZRDangerScore() >= 4) return true;
   if(GetZRDangerScore() >= 5) return true;
   return false;
}

ENUM_ZR_ANCHOR_MODE ZREffectiveAnchorMode()
{
   if(InpZRAdaptiveScheme == ZR_SCHEME_MANUAL)             return InpZoneAnchorMode;
   if(InpZRAdaptiveScheme == ZR_SCHEME_FIXED_FIRST)        return ZR_ANCHOR_FIRST;
   if(InpZRAdaptiveScheme == ZR_SCHEME_ROLLING_EXPANDING)  return ZR_ANCHOR_ROLLING;
   return (ZRUseDefenseScheme() ? InpZRDefenseAnchorMode : InpZRStandardAnchorMode);
}

double ZREffectiveZoneFactor()
{
   double factor = InpZoneWidthFactor;
   if(InpZRAdaptiveScheme == ZR_SCHEME_FIXED_FIRST)       factor = InpZRStandardZoneFactor;
   else if(InpZRAdaptiveScheme == ZR_SCHEME_ROLLING_EXPANDING) factor = InpZRDefenseZoneFactor;
   else if(InpZRAdaptiveScheme == ZR_SCHEME_AUTO_STATE)   factor = (ZRUseDefenseScheme() ? InpZRDefenseZoneFactor : InpZRStandardZoneFactor);

   if(factor <= 0.0) factor = 1.0;
   return factor;
}

string ZRActiveSchemeName()
{
   if(InpZRAdaptiveScheme == ZR_SCHEME_MANUAL) return "MANUAL";
   if(InpZRAdaptiveScheme == ZR_SCHEME_FIXED_FIRST) return "FIXED_FIRST";
   if(InpZRAdaptiveScheme == ZR_SCHEME_ROLLING_EXPANDING) return "ROLLING_EXPANDING";
   return (ZRUseDefenseScheme() ? "AUTO_DEFENSE_ROLLING" : "AUTO_STANDARD_FIXED");
}
...
double GetEffectiveZoneWidth(double base_width, int step_index)
{
   ...
   double factor = ZREffectiveZoneFactor();
   if(factor <= 0.0) return base_width;
   return base_width * MathPow(factor, step_index);
}
...
bool TryInitZoneRecovery(ENUM_AMOS_MARKET_STATE state, ENUM_AMOS_STRATEGY_SLOT slot)
{
   ...
   ZRLog("ZR_ARMED", "initial_position_registered|scheme=" + ZRActiveSchemeName(), state, slot, g_zr.policy);
   return true;
}
...
void HandleZoneRecovery(ENUM_AMOS_MARKET_STATE state, ENUM_AMOS_STRATEGY_SLOT slot)
{
   ...
   static string s_last_scheme = "";
   string active_scheme = ZRActiveSchemeName();
   if(active_scheme != s_last_scheme)
   {
      ZRLog("ZR_SCHEME", active_scheme + "|danger=" + IntegerToString(GetZRDangerScore()), state, slot, g_zr.policy);
      s_last_scheme = active_scheme;
   }
   ...
   if(!g_zr.recovery_started)
   {
      ...
      if(trigger && CanPlaceZROrder(mid))
      {
         ...
         if(OpenZRPosition(hedge_type, lot))
         {
            ...
            g_zr.current_zone = GetZoneLevel(mid, anchor, zw, ZREffectiveZoneFactor());
            StripBasketSLTP();
            LockZROrder(mid);
   ...
   if(ZREffectiveAnchorMode() == ZR_ANCHOR_ROLLING)
   {
   ...
   int new_zone = GetZoneLevel(mid, anchor, zw, ZREffectiveZoneFactor());
   ...
   double boundary_price = ZRZoneBoundaryPrice(anchor, zw, ZREffectiveZoneFactor(), new_zone);
   double lot = CalculateZRLot(add_type, g_zr.last_lot);
   if(OpenZRPosition(add_type, lot))
   {
      ClearZROrderFreeze("order_success");
...
#endif // __AMOS_ZR_RESCUE_MODULE_V1_02__
//+------------------------------------------------------------------+
