...
| Numeric AI direction model (LightGBM, walk-forward) | `docs/inbox/ai_direction_model_verification.md` · `scripts/ai_direction_features.py` · `scripts/ai_direction_train.py` | **No measured edge** — AUC 0.47-0.57 across 10 fold/horizon combinations (1h and 4h), indistinguishable from random. Not proceeding to ONNX/EA integration on this model. Requested to check a 3rd-party sales page's claims (PF 1.66, Sharpe 4.57) — not reproduced or explained. |
