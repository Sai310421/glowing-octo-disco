...
| Regime-gate walk-forward | `scripts/trend_rider_regime_wf.py` · `reports/trend_rider_regime_wf.json` | **Causal ER gate works**: weak months −$148/mo → +$26/mo net (+$60 w/CB) with fixed config; 9/12 configs beat baseline; per-fold re-optimization is where overfitting lives. Oracle ceiling +$350/mo. |
